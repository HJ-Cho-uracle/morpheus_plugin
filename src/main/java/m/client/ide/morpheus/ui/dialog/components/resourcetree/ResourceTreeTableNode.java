...
public class ResourceTreeTableNode extends AbstractCheckTreeTableNode {

    public ResourceTreeTableNode(String name, Object userObject) {
    ...
    protected void createChildren() {
        ...
        if (userObject instanceof File && ((File) userObject).isDirectory()) {
            ...
            for (int i = 0; i < children.length; i++) {
                ...
                add(new ResourceTreeTableNode(child.getName(), child));
            }
            ...
            for (int i = 0; i < children.length; i++) {
                ...
                add(new ResourceTreeTableNode(((PsiFileSystemItem) child).getName(), child));
            }
        }
    }
    ...
    public static class ResourceColumnInfo extends ColumnInfo {
        private final ResourceTreeTableModel.ResourceColumn column;

        public ResourceColumnInfo(ResourceTreeTableModel.@NotNull ResourceColumn name) {
            super(name.toString());
            this.column = name;
        }
        ...
        public Object valueOf(Object object) {
            if (object instanceof ResourceTreeTableNode) {
                ResourceTreeTableNode node = (ResourceTreeTableNode) object;
                switch (column) {
                    case NAME:
