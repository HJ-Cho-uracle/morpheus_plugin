...
import m.client.ide.morpheus.core.utils.MostlySilentColoredProcessHandler;
    ...
    try {
      ...
      listener = new ProcessAdapter() {
        ...
        public void processTerminated(@NotNull ProcessEvent event) {
          ...
          if (exitCode != 0) {
            final String message = stdout.toString().isEmpty()
                                   ? "Android emulator terminated with exit code " + exitCode
                                   : stdout.toString().trim();
            MorpheusNotifier.showError("Error Opening Emulator", message, androidSdk.project);
          }
        }
      };
      ...
      MorpheusNotifier.showError("Error Opening Emulator", e.toString(), androidSdk.project);
    }
  }
